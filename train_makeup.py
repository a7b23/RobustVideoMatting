...
class Trainer:
    ...
    def parse_args(self):
        ...
        # Matting dataset
        parser.add_argument('--dataset', type=str, required=True, choices=['videomakeup', 'imagematte'])
        # Learning rate
        ...
        parser.add_argument('--resolution-lr', type=int, default=256)
    ...
    def init_datasets(self):
        ...
        # Maekup datasets:
        if self.args.dataset == 'videomakeup':
            self.dataset_lr_train = VideoMakeupDataset(
                videomakeup_dir=DATA_PATHS['videomakeup']['train'],
        ...
        self.dataset_seg_video = VideoMakeupDataset(
                videomakeup_dir=DATA_PATHS['videomakeup']['train'],
                size=self.args.resolution_lr,
                seq_length=self.args.seq_length_lr,
                seq_sampler=TrainFrameSampler(),
    ...
    def train(self):
        for epoch in range(self.args.epoch_start, self.args.epoch_end):
            ...
            for no_mkp, true_mkp in tqdm(self.dataloader_lr_train, disable=self.args.disable_progress_bar, dynamic_ncols=True):
                # Low resolution pass
                self.train_makeup(no_mkp, true_mkp, downsample_ratio=1, tag='lr')
    ...
    def train_makeup(self, no_mkp, true_mkp, downsample_ratio, tag):
        no_mkp = no_mkp.to(self.rank, non_blocking=True)
        true_mkp = true_mkp.to(self.rank, non_blocking=True)
        no_mkp, true_mkp = self.random_crop(no_mkp, true_mkp)
        true_src = no_mkp
        
        with autocast(enabled=not self.args.disable_mixed_precision):
            pred_mkp = self.model_ddp(true_src, downsample_ratio=downsample_ratio)[0]
            loss = makeup_loss(pred_mkp, true_mkp)
        ...
        if self.rank == 0 and self.step % self.args.log_train_images_interval == 0:
            self.writer.add_image(f'train_{tag}_pred_mkp', make_grid(pred_mkp.flatten(0, 1), nrow=pred_mkp.size(1)), self.step)
            self.writer.add_image(f'train_{tag}_true_mkp', make_grid(true_mkp.flatten(0, 1), nrow=true_mkp.size(1)), self.step)
            self.writer.add_image(f'train_{tag}_np_mkp', make_grid(no_mkp.flatten(0, 1), nrow=no_mkp.size(1)), self.step)
    ...
    def validate(self):
        if self.rank == 0:
            ...
            with torch.no_grad():
                with autocast(enabled=not self.args.disable_mixed_precision):
                    for no_mkp, true_mkp in tqdm(self.dataloader_valid, disable=self.args.disable_progress_bar, dynamic_ncols=True):
                        no_mkp = no_mkp.to(self.rank, non_blocking=True)
                        true_mkp = true_mkp.to(self.rank, non_blocking=True)
                        true_src = no_mkp
                        batch_size = true_src.size(0)
                        pred_mkp = self.model(true_src)[0]
                        total_loss += makeup_loss(pred_mkp, true_mkp)['total'].item() * batch_size
